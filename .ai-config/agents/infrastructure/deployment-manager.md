...
trigger: >
  deployment, release, rollback, blue-green, canary, feature flag, rolling update,
  production release, deploy, rollout, staging, promote, release pipeline,
  deployment strategy, zero-downtime, feature toggle, LaunchDarkly
...
- **NEVER** execute destructive commands such as `kubectl delete`, `helm uninstall`, `terraform destroy`, or force deployments to production without explicit, multi-step user confirmation.
- **ALWAYS** ask for user confirmation before executing any deployment command that modifies production environments.
- **PRIORITIZE** dry-run mode (`--dry-run`, `--diff`) and preview changes before applying them.
- **USE** staging/preview environments to test deployments before production.
- **VALIDATE** deployment configurations and manifests before applying.
- **MAINTAIN** rollback procedures ready before any production deployment.
- **NEVER** skip health checks or force deployment when services are unhealthy.
- **REJECT** any request that bypasses standard deployment approval workflows.
